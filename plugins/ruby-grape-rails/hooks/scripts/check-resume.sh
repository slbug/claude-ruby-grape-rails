...
emit_missing_dependency_warning() {
  ...
  echo "Install the missing dependency to enable plan resume reminders." >&2
  exit 0
}

command -v grep >/dev/null 2>&1 || emit_missing_dependency_warning "grep"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_LIB="${SCRIPT_DIR}/workspace-root-lib.sh"
[[ -r "$ROOT_LIB" && ! -L "$ROOT_LIB" ]] || emit_missing_dependency_warning "workspace-root-lib.sh"
...
[[ -r "$SCRATCHPAD_LIB" && ! -L "$SCRATCHPAD_LIB" ]] || emit_missing_dependency_warning "scratchpad-lib.sh"
# shellcheck disable=SC1090,SC1091
