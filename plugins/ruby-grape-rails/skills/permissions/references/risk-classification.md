...
| `find . -type f -name '*'`, `find . -path '*'`, `find . -maxdepth N -type f`, `grep *`, `rg *`, `ag *` | Read-only search (CC 2.1.113: broad bare-`find` patterns no longer auto-approve `-exec` / `-delete`; prefer the narrow forms shown — note GNU find requires the path argument before any expression) |
