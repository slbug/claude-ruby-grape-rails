...
| `{STACK_HEADER}` | Compose from `detect-stack` `*_VERSION` keys. Always emit `Ruby <RUBY_VERSION>`. For each of `RAILS_VERSION`, `GRAPE_VERSION`, `SIDEKIQ_VERSION`, `KARAFKA_VERSION`, `HOTWIRE_VERSION`, `SEQUEL_VERSION`, `ACTIVERECORD_VERSION`: when `detect-stack` emits the key, append `, <Stack> <version>` to the header. Omit when key absent. NEVER emit a bare label without version (e.g. literal `Rails` followed by empty). `detect-stack` does NOT pre-compose this header — `/rb:init` derives it from the individual keys. | `Ruby 3.4.7, Rails 7.1.3, Grape 2.0.0, Sidekiq 7.2.0, Karafka 2.5.8` |
