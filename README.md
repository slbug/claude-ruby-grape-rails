...
| Ruby | 3 | Always pair method_missing with respond_to_missing?; Always supervise background processes; Never rescue `Exception` (or `::Exception`) in `begin/rescue` or Rails `rescue_from` — bare `rescue` is not a Law 18 violation |
